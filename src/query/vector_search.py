...
hop that can be missing credentials or simply down. When that happens tier 3 must
decline and let tier 4 run, not 500: a degraded answer beats an error page.
"""
...
class VectorSearch:
    ...
    def search(
        self, ctx: AuthContext, question: str, *, top_k: int = 20
    ) -> list[dict[str, Any]]:
        try:
            hits = self._embedder.search(ctx, question, top_k=top_k)
